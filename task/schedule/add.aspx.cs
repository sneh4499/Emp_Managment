...
public partial class task_schedule_add : System.Web.UI.Page
{
                        ...
                        if (Request.QueryString["id"].ToString() != null)
                        {
                           ...
                           fromdate = dr["fromdate"].ToString();
                            ...
                            //assignemp.Text = (dr["assignedemps"].ToString());
                             teamleader=expel.genrateemployeenames( dr["teamleader"].ToString());

                            emp1 = expel.genrateemployeenames(dr["assignedemps"].ToString());
    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
        ...
        con.Open();
        cmd = new SqlCommand("insert into schedule (datetime,sitename,workname,todate,fromdate,location,assignedemps,teamleader,remark,uploadby,status,Jobcode,Jobdetail) values (@datetime,@sitename,@workname,@todate,@fromdate,@location,@assignedemps,@teamleader,@remark,@uploadby,@status,@Jobcode,@Jobdetail)", con);
        ...
        cmd.Parameters.AddWithValue("@teamleader", drdtruck.SelectedItem.Text);
 ...
 expel.insert("calenders", dropsitename.SelectedItem.Text + " : " + drdjobcode.SelectedItem.Text, expel.getSingleData("select id from schedule where id=" + aaa), txtfromdate.Text.ToString(), a, "", txttodate.Text.ToString());		


       // expel.insert("calenders", dropsitename.SelectedItem.Text + " : " + drdworkname.SelectedItem.Text, expel.getSingleData("select id from schedule where id="+aaa), txtfromdate.Text.ToString(), a, "", txttodate.Text.ToString());
    ...
    protected void btnupdate_Click(object sender, EventArgs e)
    {
        ...
        id1 = Request.QueryString["id"].ToString();
        con.Open();
        cmd = new SqlCommand("Update schedule SET sitename=@sitename,workname=@workname,todate=@todate,fromdate=@fromdate,location=@location,assignedemps=@assignedemps,teamleader=@teamleader,remark=@remark,Jobdetail=@Jobdetail where id=@id", con);
        cmd.Parameters.AddWithValue("@id", id1);
        cmd.Parameters.AddWithValue("@sitename", dropsitename.SelectedItem.Text);
        cmd.Parameters.AddWithValue("@workname", drdworkname.SelectedItem.Text);
  ...
  string a = Request.Cookies["email"].Value.ToString();
        ...
        // SqlCommand cmd1 = new SqlCommand("update calenders set text='" + dropsitename.SelectedItem.Text + " : " + drdworkname.SelectedItem.Text + "',taskname='" + expel.getSingleData("select id from schedule where id=" + aaa) + "',date='" + txtfromdate.Text.ToString() + "',userid='" + a + "',enddate='" + txttodate.Text.ToString() + "' where id="+id1,con);
		 ...
		 SqlCommand cmd1 = new SqlCommand("update calenders set text='" + dropsitename.SelectedItem.Text + " : " + drdjobcode.SelectedItem.Text + "',taskname='" + expel.getSingleData("select id from schedule where id=" + aaa) + "',date='" + txtfromdate.Text.ToString() + "',userid='" + a + "',enddate='" + txttodate.Text.ToString() + "' where id=" + id1, con);
